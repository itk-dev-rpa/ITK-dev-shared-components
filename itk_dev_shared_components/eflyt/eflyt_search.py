...
def extract_cases(browser: webdriver.Chrome) -> list[Case]:
    ...
    for row in rows:
        ...
        start_column = 3
        try:
            deadline = row.find_element(By.XPATH, f"td[{start_column}]/a").text
        ...
        case_number = row.find_element(By.XPATH, f"td[{start_column + 1}]").text
        case_types_text = row.find_element(By.XPATH, f"td[{start_column + 2}]").text

        # If the case types ends with '...' we need to get the title instead
        if case_types_text.endswith("..."):
            case_types_text = row.find_element(By.XPATH, f"td[{start_column + 2}]").get_attribute("Title")
        case_types = case_types_text.split(", ")

        status = row.find_element(By.XPATH, f"td[{start_column + 3}]").text
        cpr = row.find_element(By.XPATH, f"td[{start_column + 4}]/a").text
        name = row.find_element(By.XPATH, f"td[{start_column + 5}]").text
        case_worker = row.find_element(By.XPATH, f"td[{start_column + 7}]").text
